...
echo_info "命令: sudo target/debug/examples/passthrough \\"
...
sudo "/root/rk8s/project/target/debug/examples/passthrough" \
    --rootdir "${SRC_DIR}" \
    --mountpoint "${MNT_DIR}" \
...
# Need sudo or proper permissions if mounted as root
...
echo ""
echo "  sudo cargo run --example passthrough -- \\"
