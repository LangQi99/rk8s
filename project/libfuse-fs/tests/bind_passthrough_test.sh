...
echo_step "测试 15: 只读保护验证 - 修改现有文件"
echo_info "$ 尝试修改只读 bind mount 中的现有文件（应该失败）"
if echo "append" >> "${MNT_DIR}/logs/readonly.txt" 2>&1 | grep -q "只读"; then
...
echo_step "测试 16: 只读保护验证 - 重命名/移动"
echo_info "$ 尝试重命名只读 bind mount 中的文件（应该失败）"
if mv "${MNT_DIR}/logs/readonly.txt" "${MNT_DIR}/logs/moved.txt" 2>/dev/null; then
    echo_error "只读保护失败 - 文件被重命名"
else
    echo_success "Pass - 重命名被拒绝"
fi

echo_step "测试 17: 只读保护验证 - 删除"
echo_info "$ 尝试删除只读 bind mount 中的文件（应该失败）"
if rm "${MNT_DIR}/logs/readonly.txt" 2>/dev/null; then
...
echo_success "✓ 文件同步正确"
echo_success "✓ 严格的只读权限控制 (Create/Write/Rename/Delete)"

echo ""
echo_step "使用方式"
echo_info "标准的 passthrough_example 现在支持 --bind 参数："
echo ""
echo "  sudo cargo run --example passthrough_example -- \\"
