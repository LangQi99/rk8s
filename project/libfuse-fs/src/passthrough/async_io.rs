...
use super::{
    Handle, HandleData, PassthroughFs, ROOT_ID, config::CachePolicy, os_compat::LinuxDirent64,
    util::*,
};
...
impl<S: BitmapSlice + Send + Sync> PassthroughFs<S> {
    ...
    async fn do_readdir(
        ...
        loop {
            ...
            while offset < bytes_read {
                ...
                if name.to_str().unwrap().contains("bind") {
                    info!(
                        "do_readdir: processing bind entry: inode={}, name={:?}",
                        inode, name
                    );
                }
        ...
        // If reading root directory, inject bind mount entries
        if inode == ROOT_ID {
            let bind_mounts = self.bind_mount_manager.get_all_mounts().await;
            for bind_mount in bind_mounts {
                let mount_point_name = bind_mount
                    .mount_point
                    .file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("");
                ...
                if !mount_point_name.is_empty() {
                    // Lookup the bind mount to get its attributes
                    let name_cstr = CString::new(mount_point_name)
                        .map_err(|_| io::Error::from_raw_os_error(libc::EINVAL))?;

                    if let Ok(_entry) = self.do_lookup(inode, &name_cstr).await {
                        let entry = DirectoryEntry {
                            inode: _entry.attr.ino,
                            kind: FileType::Directory,
                            name: OsString::from(mount_point_name),
                            offset: entry_list.len() as i64 + 1000,
                        };
                        entry_list.push(Ok(entry));
                    }
                }
    ...
    async fn do_open(&self, inode: Inode, flags: u32) -> io::Result<(Option<Handle>, OpenOptions)> {
        if (flags as i32 & libc::O_ACCMODE) != libc::O_RDONLY || (flags as i32 & libc::O_TRUNC) != 0
        {
            self.check_write_permission(inode).await?;
        }
...
impl Filesystem for PassthroughFs {
    ...
    async fn setattr(
        ...
        // Check if inode is in readonly bind mount
        self.check_write_permission(inode).await?;
    ...
    async fn rename(
        ...
        // Check write permission for both parent and new_parent
        self.check_write_permission(parent).await?;
        if parent != new_parent {
            self.check_write_permission(new_parent).await?;
        }
        ...
        // Also check if the file being renamed is in a readonly bind mount
        match self.do_lookup(parent, oldname).await {
            Ok(entry) => {
                self.check_write_permission(entry.attr.ino).await?;
            }
            Err(_) => {
                // If lookup fails, the file doesn't exist, which will be caught by renameat
            }
        }
    ...
    async fn rename2(
        ...
        match self.do_lookup(parent, oldname).await {
            ...
            Err(_) => {
                // If lookup fails, the file doesn't exist, which will be caught by renameat2
            }
        }
