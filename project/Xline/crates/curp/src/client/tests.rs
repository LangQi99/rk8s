    ...
    let connects = init_mocked_connects(3, |_id, conn| {
        conn.expect_fetch_cluster().return_once(|_req, _timeout| {
            Ok(tonic::Response::new(FetchClusterResponse {
                ...
                cluster_id: 123,
                members: vec![
                    Member::new(0, "S0".to_owned(), vec!["A0".to_owned()], vec![], false),
                    Member::new(1, "S1".to_owned(), vec!["A1".to_owned()], vec![], false),
                    Member::new(2, "S2".to_owned(), vec!["A2".to_owned()], vec![], false),
                ],
                cluster_version: 1,
            }))
        });
                ...
                let resp = match id {
                    0 => FetchClusterResponse {
                        ...
                        members: vec![
                            ...
                            Member::new(1, "S1".to_owned(), vec!["A1".to_owned()], vec![], false),
                            Member::new(2, "S2".to_owned(), vec!["A2".to_owned()], vec![], false),
                            Member::new(3, "S3".to_owned(), vec!["A3".to_owned()], vec![], false),
                            Member::new(4, "S4".to_owned(), vec!["A4".to_owned()], vec![], false),
                        ],
                        cluster_version: 1,
                    },
                    ...
                    3 => FetchClusterResponse {
                        ...
                        cluster_id: 123,
                        members: vec![
                            Member::new(0, "S0".to_owned(), vec!["B0".to_owned()], vec![], false),
                            Member::new(1, "S1".to_owned(), vec!["B1".to_owned()], vec![], false),
                            Member::new(2, "S2".to_owned(), vec!["B2".to_owned()], vec![], false),
                            Member::new(3, "S3".to_owned(), vec!["B3".to_owned()], vec![], false),
                            Member::new(4, "S4".to_owned(), vec!["B4".to_owned()], vec![], false),
                        ],
                        cluster_version: 1,
                    },
